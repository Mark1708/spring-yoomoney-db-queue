...
@Configuration
@EnableConfigurationProperties(value = {DbQueueProperties.class, QueueConfigurationProperties.class})
...
public class DbQueueAutoConfiguration {
    ...
    public String queueName() {
        ...
        return dbQueueProperties.getConfig().keySet().stream()
                .findFirst()
    ...
    public SpringDatabaseAccessLayer databaseAccessLayer(
        ...
        return new SpringDatabaseAccessLayer(QueueTableSchema.builder().build(), jdbcOperations, transactionOperations);
    }

    @Bean
    public List<QueueShard<DatabaseAccessLayer>> queueShards(
            DatabaseAccessLayer databaseAccessLayer, QueueConfigurationProperties configurationProperties) {
        return configurationProperties.getShards().stream()
                .map(shard -> new QueueShard<>(new QueueShardId(shard), databaseAccessLayer))
                .toList();
    }
    ...
    @Bean
    @ConditionalOnMissingBean
    public QueueSettings queueSettings(QueueConfigurationProperties configurationProperties) {
    ...
    @Bean
    @ConditionalOnMissingBean
    public QueueLocation queueLocation(QueueId queueId, QueueConfigurationProperties configurationProperties) {
    ...
    @Bean
    @ConditionalOnMissingBean
    public QueueConfig queueConfig(QueueLocation queueLocation, QueueSettings queueSettings) {

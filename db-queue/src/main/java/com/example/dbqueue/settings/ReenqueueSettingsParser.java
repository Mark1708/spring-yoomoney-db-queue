...
class ReenqueueSettingsParser {
    ...
    private static ReenqueueRetryType parseReenqueueRetryType(String type) {
        return switch (type) {
            case QueueConfigsReader.VALUE_REENQUEUE_RETRY_TYPE_MANUAL -> ReenqueueRetryType.MANUAL;
            case QueueConfigsReader.VALUE_REENQUEUE_RETRY_TYPE_FIXED -> ReenqueueRetryType.FIXED;
            case QueueConfigsReader.VALUE_REENQUEUE_RETRY_TYPE_SEQUENTIAL -> ReenqueueRetryType.SEQUENTIAL;
            case QueueConfigsReader.VALUE_REENQUEUE_RETRY_TYPE_ARITHMETIC -> ReenqueueRetryType.ARITHMETIC;
            case QueueConfigsReader.VALUE_REENQUEUE_RETRY_TYPE_GEOMETRIC -> ReenqueueRetryType.GEOMETRIC;
            default -> throw new IllegalArgumentException(String.format("unknown reenqueue retry type: type=%s", type));
        };
    }
